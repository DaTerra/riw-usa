...
$Modulo = array(
	'titulo'=>'Meeting',
	'tabela'=>'tbmeeting_convites',
	'modulo'=>'meeting',
	'pasta'=>'',
	'acao'=>$_GET['a'],
	'id_tabela'=>'id_convite',
	'icone'=>'',
);
...
<?
	...
	/*
		...
		if(!empty($_GET['fil_first'])){
		 ...
		 $SQL = 'SELECT * FROM tbmeeting_convites
				...
				INNER JOIN tbmeeting_table ON tbmeeting_convites.id_table=tbmeeting_table.id_table 
				WHERE tbmeeting_convites.status=1';
		 ...
		 if($_GET['campo']){
			 ...
			 $SQL .= ' ORDER BY tbmeeting_convites.id_convite  ASC';
		}
...
<div class="limpar"></div>
    <thead>
    	<tr>
        	<td class="left"><a href="?m=<?=$Modulo['modulo']?>&amp;campo=tbmeeting_day.id_day&amp;order=<?=$order?><?=$url_busca?>" <? if($_GET['campo']=='tbmeeting_day.id_day' or empty($_GET['campo'])) echo' class="'.$class_order.'"';?>>Date</a></td>
        	<td class="left"><a href="?m=<?=$Modulo['modulo']?>&amp;campo=tbmeeting_instantes.id_instante&amp;order=<?=$order?><?=$url_busca?>" <? if($_GET['campo']=='tbmeeting_instantes.id_instante' or empty($_GET['campo'])) echo' class="'.$class_order.'"';?>>Start</a></td>
        	<td class="left">End</td>
        	<td class="left">Description</td>
        	<td class="left"><a href="?m=<?=$Modulo['modulo']?>&amp;campo=tbmeeting_table.id_table&amp;order=<?=$order?><?=$url_busca?>" <? if($_GET['campo']=='tbmeeting_table.id_table' or empty($_GET['campo'])) echo' class="'.$class_order.'"';?>>Table</a></td>
...
*/?>
        <? 
			...
			if($DB->nun_linhas($lista)>0){
				while($linha = $DB->lista($lista)){
						...
						$convidado = $DB->dados('SELECT * FROM tbuser WHERE id_user='.(int)$linha['id_convidado'].' LIMIT 1');
						$solicitante = $DB->dados('SELECT * FROM tbuser WHERE id_user='.(int)$linha['id_solicitante'].' LIMIT 1');
		...
		?>
        <tr>
            <td class="left"><?=$linha['day']?></td>        
            <td class="left"><?=$linha['instante']?></td>        
            <td class="left"><?=$instante['instante']?></td>        
            <td class="left"><?=$convidado['first_name']?> <?=$convidado['last_name']?>, <? if(!empty($convidado['title'])) echo $convidado['title'].',';?> <?=$convidado['company']?> | <?=$solicitante['first_name']?> <?=$solicitante['last_name']?>, <? if(!empty($solicitante['title'])) echo $solicitante['title'].',';?> <?=$solicitante['company']?></td>        
            <td class="left"><?=$mesa['table']?></td>        
