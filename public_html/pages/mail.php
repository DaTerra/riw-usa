	...
	foreach($_POST['selected'] as $v){
		...
		if($_POST['email_action']=='intite'){	
			...
			if($Dados['id_status']=='3'){
				...
				$DB->insereArray( array('tabela'=>'tbhistory', 'id_tabela'=>'id_user'), array('id_status'=>1), 'atualiza', $v);
				
				
				header('Location: ../sistema.php?m=invitation&a=lista&S='.urlencode('Email enviado com sucesso'));exit;
			}
			...
			if($Dados['id_status']=='2'){
				...
				$corpo_mensagem = str_replace('{[FirstName][LastName]}', $indicador['first_name'],$corpo_mensagem);
				$corpo_mensagem = str_replace('{[Nome do Referee]}', $Dados['first_name'].' '.$Dados['last_name'],$dados_mail['email']);
				$emailHtml = $corpo_mensagem;
				...
				$DB->insereArray( array('tabela'=>'tbuser_keys'), array('id_user'=>$v, 'key'=>$key, 'usada'=>0), 'cadastra', $ID);
				$DB->insereArray( array('tabela'=>'tbhistory'), array('id_user'=>$v, 'action'=>'Convite enviado'), 'cadastra', $ID);
		...
		// Emails de negaчуo
			...
			if($Dados['id_status']=='3'){
				...
				header('Location: ../sistema.php?m=invitation&a=lista&S='.urlencode('Email(s) de negaуo enviado(s) com sucesso'));exit;
