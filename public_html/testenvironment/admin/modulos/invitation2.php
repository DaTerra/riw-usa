...
// ================================
	...
	if(!(strlen($title)>3)) $erro.= '- Title <br />';
	...
	if(strlen($erro)>0){
		...
		header('Location: ..'.$origem.'/?erro='.urlencode('<strong>Please fill up the mandatory fields.:</strong><br />'.$erro));exit;
	...
	if(!empty($_POST['first_name1'][0])){
		for($y=0; $y<=($a-1); $y++){
			...
			$DB->insereArray( array('tabela'=>'tbhistory'), array('id_user'=>$DB->lastInsertedId(), 'action'=>"Was refeared by $first_name $last_name para receber convite"), 'cadastra', $ID);
			$DB->insereArray( array('tabela'=>'tbhistory'), array('id_user'=>$id_user, 'action'=>"Referral $nome to be invited "), 'cadastra', $ID);
...
// ============================================	
	...
	header('Location: ../alert/?msg='.urlencode("Congratulations. Your invitation has been confirmed. We are looking forward to see you!!"));exit;
?>